/*! \file      comm.cpp Contains the functions responsible for communicating with the Slow Control System
 *  \details   This file contains all of the necessary functions to communicate with the different
 * 			   subsystems within the Slow Control system.  These are used to measure the
...
/*! \brief Constructs command to turn on a Cartridge Slot on the HV Floating Board
 *
 * This constructs a vector of characters that can be sent over a usb interface to the HV Floating Board
 * to specify that a slot should be turned on or off.
 *
 * \param Slot The slot on the HV Floating Board that the power command is specified for, not the Cartridge Number
 * \param power Specifies whether the set command will set the power on or off.  (false=off) (true=off)
 *
...
std::vector<char> ConstructCartridgePowerSetCommand(int Slot, bool power) {
	...
	if ( (Slot<0) || (Slot>2) ) return (std::vector<char>());
 ...
 *
 * This constructs a vector of characters that represents the expected response of the HV Floating Board
 * from a successful power set.  This can be used to verify the actual response received matches this
 * so that the program can validate that the slot was actually set
 *
 * \param Slot The slot on the HV Floating Board that the power command is specified for, not the Cartridge Number
 * \param power Specifies whether the set command will set the power on or off.  (false=off) (true=off)
 *
...
/*! \brief Constructs command to check if a Cartridge Slot on the HV Floating Board is on
 *
 * This constructs a vector of characters that can be sent over a usb interface to the HV Floating Board
 * to query the current state of that slot
 *
 ...
 *
 * This constructs a vector of characters that can be sent over a usb interface to the SCMicros
 * the order in which the IDs have been set
 *
 * \return Vector of charcters to be sent over usb interface
 */
...
/*! \brief Constructs command to query the leakage current of module on a discrete board
 *
 * This constructs a vector of characters that can be sent over a usb interface to a discrete board,
 * or SCMicro, to query the leakage current of a specific module that is connected to the SCMicro.
 *
 * \param SCMicroID The ID of the SCMicro within the chain that should be queried
 * \param ModuleID The ID of the module local to the SCMicro that should have it's current measured
 *
 * \return Vector of charcters to be sent over usb interface
 */
 ...
 *
 * This constructs a vector of characters that can be sent over a usb interface to a Discrete Board
 * microcontroller, known as an SCMicro, to query the temperature of the thermistor that is connected
 *
 * \warning Hard Coded SCMicro ID Range
 * \warning Only half of the SCMicros have a thermistor connected and will return a valid output
 *
...
/*! \brief Constructs command to set the gain on the common of the specified PSAPD
 *
 * This constructs a vector of characters that can be sent over a usb interface to a Discrete Board
 * microcontroller, known as an SCMicro, to set the programmable capacitor to a specified value so
 * that the gain to common channel can be adjusted
 *
 * \warning Hard Coded SCMicro ID Range
 * \warning Though responing correctly Discrete Boards may not be setting values of the
 ...
 * \param Cap the value at which the capacitor should be programmed.  0 through 31 is accepted
 *
 * \return Vector of charcters to be sent over usb interface.  An empty vector for invalid values.
 */
...
/*! \brief Constructs the expected response to the command to set the gain on the common of the specified PSAPD
 *
 * This constructs a vector of characters that can compared to the response that is returned from the Discrete
 * board microcontroller, known as an SCMicro, to set the programmable capacitor to a specified value so
 * that the gain to common channel can be adjusted
 *
 * \warning Hard Coded SCMicro ID Range
 * \warning The send command requires a zero padded gain number, but the return may not be
 *
 * \param SCMicroID the ID number of the SCMicro that is being addressed
 * \param ModuleID the local ID of the Module that is being addressed
 * \param PSAPD which PSAPD on the module should be addressed
 * \param Cap the value at which the capacitor should be programmed.  0 through 31 is accepted
 *
 * \return Vector of charcters for comparison.  An empty vector for invalid values.
 */
...
/*! \brief Constructs the command chain to set the IDs of the microcontrollers in a specific order
 *
 * The discrete boards are programmed with <US#######> command.  The microcontroller takes the first
 * value off of the chain as its ID and send the command, sans that ID to the next microcontroller.
 * If there is only one number left, then this number is decremented in in ASCII form and then the
 * command is sent to the next microcontroller in the chain.  If there are no numbers supplied, then
 * the ID is assumed to be 'F' by the first microcontroller in the chain, and it then provides the
 * appropritely decremented command of '\<USE\> ' to the next micro.
 *
 * \warning Hard Coded SCMicro ID Range
 *
 * \param SCMicroIDs a vector of IDs in which the chain of micros should be set.
 *
 * \return Vector of charcters to send over the usb interface.  An empty vector for invalid values.
 */
...
/*! \brief Constructs command to begin set ID autodecrement in SCMicros starting with 'F'
 *
 * Overloaded function that uses ConstructIDSetCommand(std::vector<int>) to construct '\<US\> '
 * which begins settings the microcontroller IDs at 'F' and then begins to autodecrement from
 * there
 *
 * \return Vector of charcters to send over the usb interface.  An empty vector for invalid values.
 */
...
/*! \brief Constructs command to begin set ID autodecrement in SCMicros starting with a specified number
 *
 * Overloaded function that uses ConstructIDSetCommand(std::vector<int>) to construct '<US#> '
 * which begins settings the microcontroller IDs at a specified number and then begins to autodecrement
 * from there
 *
    ...
    if (abbreviated == 2) {
        ...
        if (difference > 0) {
            return(ConstructIDSetCommand(
                       Util::BuildOrderedVector(difference+1, false, DecrementStart)));
        } else {
            // Operate like normal abbreviate == 2 mode
            return(ConstructIDSetCommand(std::vector<int>(1,DecrementStart)));
        }
    } else if (abbreviated == 0) {
        return(ConstructIDSetCommand(Util::BuildOrderedVector(DecrementStart+1, false)));
    } else {
        return(std::vector<char>());
    }
}

/*! \brief Constructs command to set the DAC voltage on the HVBias board to a specified Module
 *
 * This constructs a command which is used to set a the voltage that is supplied by the DAC chips on
 * the HVBiasBoard to the specified Module.  This is a positive offset from the negative floating
 * ground that is supplied by the Crate through the HVFloatingBoard.
 *
 * \param CartridgeSlot the slot number on the HVFloatingBoard to which the Cartridge is connected.
          Accepted values of slot numbers are 0, 1, and 2.
 * \param DAC_Chip the number of the DAC chip that contains the Module whos voltage is to be
 		  programmed. Each DAC Chip can program up to 32 modules, so each DAC chip controls 2 Fins.
		  Chip numbers between 0 and 3 are accepted.
 * \param DAC_Channel The channel on which the Module to be programmed is connected. There are
          32 channels per DAC Chip so 0 through 31 are acceptable numbers.
 * \param Voltage The voltage that is to be added to the negative floating ground for the Module.
          Voltages betwen 0 and 99 are accepted.
 *
 * \return Vector of charcters to send over the usb interface.  An empty vector for invalid values.
 */
...
/*! \brief Constructs expected response to command to set the DAC voltage on the HVBias board to a specified Module
 *
 * This constructs an expected response to a successful command to set the voltage that is supplied
 * by the DAC chips on the HVBiasBoard to the specified Module.
 *
 * \param CartridgeSlot the slot number on the HVFloatingBoard to which the Cartridge is connected.
          Accepted values of slot numbers are 0, 1, and 2.
 * \param DAC_Chip the number of the DAC chip that contains the Module whos voltage is to be
 ...
 * \param Voltage The voltage that is to be added to the negative floating ground for the Module.
          Voltages betwen 0 and 99 are accepted.
 *
 * \return Vector of charcters to for comparison to response.  An empty vector for invalid values.
 */
std::vector<char> ConstructVoltageResponse(int CartridgeSlot, int DAC_Chip, int DAC_Channel, int Voltage){
	if ( (CartridgeSlot <0) || (CartridgeSlot >2) ) return (std::vector<char>());
	if ( (DAC_Chip <0) || (DAC_Chip >3) ) return (std::vector<char>());
	if ( (DAC_Channel <0) || (DAC_Channel >31) ) return (std::vector<char>());
	if ( (Voltage <0) || (Voltage >99) ) return (std::vector<char>());

	std::vector<char> ret(13);
	sprintf(&(ret[0]),"<v%02d%02du%1dc%1d> ",DAC_Channel,Voltage,DAC_Chip,CartridgeSlot);
	ret.pop_back();
	return(ret);
}

/*! \brief Parses a received response to a temperature query command for information
 *
 * This parses the response that is received in response to a request for the temperature on a
 * thermistor connected to a Discrete Board, or SCMicro.  This is done by verifying the message
 * is in the proper format, with opening and closing brackets, proper placement of the parameters
 * within the message, including 'u' for the microcontroller ID, and 't' for the start of the
 * temperature. If the message is verified to be in the proper format, then the SCMicroID,
 * HighNumber, and LowNumber are modified with the values that the message contained.  These
 * can then be parsed into physical numbers using the calibration file.
 *
 * \warning If the given vector of characters contains two messages or more than just a
            single message, the key characters from the end of the vector will be selected
			and an error will most likely be produced.
 *
 * \param rxv The received vector of characters to be parsed
 * \param SCMicroID The integer in which the ID of the sendign SCMicro will be placed
 * \param HighNumber The integer in which the high number from the thermistor chip will be placed
 * \param LowNumber The integer in which the low number from the thermistor chip will be placed
 *
 * \return An integer value from Parse_Status indicating a successful parse or the type of error
 */
...
/*! \brief Parses a received response to a leakage current query command for information
 *
 * This parses the response that is received in response to a request for the leakage current
 * on a module connected to a Discrete Board, or SCMicro.  This is done by verifying the message
 * is in the proper format, with opening and closing brackets, proper placement of the parameters
 * within the message, including 'u' for the microcontroller ID, 'm' for the module number
 * and 'i' for the start of the leakage current. If the message is verified to be in the
 * proper format, then the SCMicroID, LocalModuleID, and Current variables supplied to the
 * function are modified with the values that the message contained.  These can then be
 * converted into physical numbers using the calibration file.
 *
 * \warning If the given vector of characters contains two messages or more than just a
            single message, the key characters from the end of the vector will be selected
			and an error will most likely be produced.
 *
 * \param rxv The received vector of characters to be parsed
 * \param SCMicroID The integer in which the ID of the sending SCMicro will be placed
 * \param LocalModuleID The integer in which the ID of the module addressed will be placed
 * \param Current The integer in which the ADC value of the current will be placed
 *
 * \return An integer value from Parse_Status indicating a successful parse or the type of error
 */
int ParseLCResponse(
...
/*! \brief Parses a received response to a cartridge power query command for information
 *
 * This parses the response that is received in response to a request for the status
 * of power to a cartridge slot that is on an HVFloatingBoard.  This is done by verifying
 * the message is in the proper format, with opening and closing brackets, proper placement
 * of the parameters within the message, including '?' for the query command, which is
 * followed by the cartridge slot number. If the message is verified to be in the
 * proper format, then the CartridgeSlot and Status variables supplied to the function
 * are modified with the values that the message contained.
 *
 * \warning If the given vector of characters contains two messages or more than just a
            single message, the key characters from the end of the vector will be selected
			and an error will most likely be produced.
 *
 * \param rxv The received vector of characters to be parsed
 * \param CartridgeSlot The integer where the Slot Number of a Valid message is placed
 * \param Status  The bool in which the status of a valid message will be returned
 *
 * \return An integer value from Parse_Status indicating a successful parse or the type of error
 */
...
/*! \brief Parses a received response to a cartridge power set command for information
 *
 * This parses the response that is received in response to a command to set the
 * power to a cartridge slot that is on an HVFloatingBoard.  This is done by verifying
 * the message is in the proper format, with opening and closing brackets, proper placement
 * of the parameters within the message, including 'p' for the power set command, which is
 * followed by the cartridge slot number. If the message is verified to be in the
 * proper format, then the CartridgeSlot and Status variables supplied to the function
 * are modified with the values that the message contained.
 *
 ...
 * \param uppercase An optional flag to tell the function to return uppercase or lowercase
          hex values.  The default value is true, or uppercase.
 *
 * \return An ASCII character representing the hex of the integer. -1 is returned for
...
/*! \brief Parses a received response to a set DAC voltage command for information
 *
 * This parses the response that is received in response to a command to set the DAC voltage
 * on a module connected to a specific DAC chip on an HV Bias Board.  This is done by
 * verifying the message is in the proper format, with opening and closing brackets, proper
 * placement of the parameters within the message, including 'c' for the cartridge slot,
 * 'u' for the DAC Chip ID, and 'v' for the voltage. If the message is verified to be in the
 * proper format, then the RxCartridgeSlot, RxDACID, RxDACAddress, and RxDACVoltage,
 * variables supplied to the function are modified with the values that the message contained.
 *
 * \warning If the given vector of characters contains two messages or more than just a
            single message, the key characters from the end of the vector will be selected
			and an error will most likely be produced.
 *
 * \param rxv The received vector of characters to be parsed
 * \param RxCartridgeSlot The integer in which the ID of the cartridge slot will be placed
 * \param RxDACID The integer in which the ID of the DAC Chip addressed will be placed
 * \param RxDACAddress The integer in which the Address of the DAC chip set will be placed
 * \param RxDACVoltage The integer in which the voltage set by the DAC will be placed
 *
 * \return An integer value from Parse_Status indicating a successful parse or the type of error
 */
